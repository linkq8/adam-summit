...
func run() -> void:
	check(Update.newer("v0.7.1") and Update.newer("v1.0.0"), "Version comparison")
	check(not Update.newer("v0.7.0") and not Update.newer("v0.6.9") and not Update.newer("oops"), "No downgrade or malformed version")
