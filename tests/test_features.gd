...
func _initialize() -> void:
	...
	solo.players[0].branch_hits[fork] = true
	solo.players[0].branch_collected[fork] = true
	...
	check(restored.players[0].branch_hits.has(fork) and restored.players[0].stars == 3, "Save retains branch progress and reward")
	...
	m.cooperative = true
	p = m.players[0]
	p.p = Vector2(m.platform_x(8), m.platforms[8].y - 1); p.v = Vector2(0, 120)
	p.highest = 7; p.camera = p.p.y - 345
	m.step(1.0 / 60, Vector2.ZERO)
	check(m.players[1].checkpoint == 8 and m.players[1].shield > 0, "Cooperation shares checkpoint and protection")
	p.finish = 10
